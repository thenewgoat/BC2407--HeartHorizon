...
# The 2 key variables we want to predict are DURATION_OF_STAY and DURATION_OF_INTENSIVE_UNIT_STAY
...
ggplot(data, aes(x = "", fill = RURAL)) +
  geom_bar(width = 1, stat = "count") +
  coord_polar(theta = "y") +
  labs(fill = "", x = NULL, y = NULL, title = "Patient's Area of Stay (Rural or Urban)") +
...
ggplot(data, aes(x = "", fill = `ADMISSION_WAY`)) +
  geom_bar(width = 1, stat = "count") +
  coord_polar(theta = "y") +
  labs(fill = "", x = NULL, y = NULL, title = "Distribution on Patient Type of Admission") +
...
# Bar Plot of duration of stay by gender
ggplot(data, aes(x = DURATION_OF_STAY, fill = GENDER)) +
  geom_bar(position = "dodge") +
  labs(title = "Bar Plot of Duration of Stay by Gender", x = "Duration of Stay", y = "Count") +
  theme_minimal()
...
# Bar Plot of duration of stay by admission way
ggplot(data, aes(x = DURATION_OF_STAY, fill = ADMISSION_WAY)) +
  geom_bar(position = "dodge") +
  labs(title = "Bar Plot of Duration of Stay by Admission Way", x = "Duration of Stay", y = "Count") +
  theme_minimal()
...
# Bar Plot of duration of stay by rural
ggplot(data, aes(x = DURATION_OF_STAY, fill = RURAL)) +
  geom_bar(position = "dodge") +
  labs(title = "Bar Plot of Duration of Stay by Rural", x = "Duration of Stay", y = "Count") +
  theme_minimal()

# Bar Plot of duration of ICU stay by rural
...
# Scatter plot of duration of stay vs age
ggplot(data, aes(x = TLC, y = DURATION_OF_STAY)) +
  geom_point() +
  labs(title = "Scatter Plot of Duration of Stay vs. TLC", x = "TLC", y = "Duration of Stay") +
  theme_minimal()

# Scatter plot of duration of stay vs age
ggplot(data, aes(x = TLC, y = DURATION_OF_ICU_STAY)) +
  geom_point() +
  labs(title = "Scatter Plot of Duration of ICU Stay vs. TLC", x = "TLC", y = "Duration of Stay") +
  theme_minimal()
...
# 1. Anaemia
# -------------------------------------------------------------
sum(data$`SEVERE_ANAEMIA` == 1) # 303 rows
sum(data$`ANAEMIA` == 1) # 2763 rows
sum(data$`SEVERE_ANAEMIA` == 1 & data$`ANAEMIA` == 1) #303 rows
...
sum(data$`HEART_FAILURE` == 1) # 4523 rows
sum(data$`HFREF` == 1) # 2403 rows
sum(data$`HFNEF` == 1) # 2132 rows
2403 + 2132 # 4535, issue because there are more rows than HEART FAILURE == 1


sum(data$`HFNEF` == 1 & data$`HFREF` == 1) # 7 rows
# Issue, logic error, cant have normal and reduced EF at the same time
# drop these 7 rows first
...
sum(data$`HEART_FAILURE` == 0 & data$`HFNEF` == 1) # 3 rows

data$`HEART_FAILURE`[data$`HEART_FAILURE` == 0 & data$HFREF == 1] <- 1
data$`HEART_FAILURE`[data$`HEART_FAILURE` == 0 & data$HFNEF == 1] <- 1

sum(data$`HEART_FAILURE` == 1) # 4521 rows
sum(data$`HFREF` == 1) # 2396 rows
sum(data$`HFNEF` == 1) # 2125 rows
2396 + 2125 # 4521 rows, the column results tally
